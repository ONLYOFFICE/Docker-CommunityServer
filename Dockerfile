...
RUN echo "${SOURCE_REPO_URL}" >> /etc/apt/sources.list && \
	...
	mkdir -p /usr/lib/jvm && \
	tar -xf jdk-8u161-linux-x64.tar.gz -C /usr/lib/jvm && \
	...
	update-alternatives --install /usr/bin/javaws javaws /usr/lib/jvm/jdk1.8.0_161/bin/javaws 1 && \
	update-alternatives --set java /usr/lib/jvm/jdk1.8.0_161/bin/java && \
	update-alternatives --set javac /usr/lib/jvm/jdk1.8.0_161/bin/javac && \
	update-alternatives --set javaws /usr/lib/jvm/jdk1.8.0_161/bin/javaws && \
